...
def load_scan(path):
    '''
    reads dicom files and return dicom slices (their format still)
    '''
    print(path)
...
def get_pixels_hu(slices):
    '''
    takes slices and returns np.array of the slices
    '''
...
if __name__ == '__main__':
    ...
    parser.add_argument("--inputFolder", type=str, help="path to the root folder", default="./stage1")
    ...
    patients = os.listdir(args.inputFolder)
    patients.sort()

    no_slicies = args.nSlices
    print("Loading first patient")
    first_patient        = load_scan(args.inputFolder+ patients[0])
    first_patient_pixels = get_pixels_hu(first_patient)
    first_patient_pixels = normalize(first_patient_pixels, args.MIN_BOUND, args.MAX_BOUND)
